...
export async function createPr(repoPath: string, branch: string, taskId: string, approved: boolean = false) {
    // 1. Prepare Content
...
`);
    ...
    if (!approved) {
        ...
        if (!fs.existsSync(prdPath)) prdPath = path.join(repoPath, "prd.md");
