...
import { run_cmd, Style, printBanner, formatTime, spawn_cmd } from './pickle_utils.js';
...
async function main() {
    ...
    for (let i = 0; i < tasks.length; i++) {
        ...
        try {
            ...
            const res = run_cmd(
                ["node", path.join(scriptsDir, "git_utils.js"), "add", "--repo", repoPath, "--branch", baseBranch, "--id", taskId],
                { capture: true }
            );
...
`);
            ...
            if (worktreePath && prApproved) {
                ...
                run_cmd(
                    ["node", path.join(scriptsDir, "git_utils.js"), "remove", "--path", worktreePath],
                    { check: false }
                );
