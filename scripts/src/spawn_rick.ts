...
function initializeSession(taskDir: string, worktreePath: string): string {
...
function getIncludes(extensionRoot: string): string[] {
...
async function main() {
    ...
    if (fs.existsSync(mapPath)) {
        try { sessionMap = JSON.parse(fs.readFileSync(mapPath, 'utf-8')); } catch {}
    }
        ...
        proc.on('close', async (code) => {
            ...
            if (isSuccess) {
                ...
                try {
                    ...
                    if (prApproved) factoryArgs.push("--approved");
