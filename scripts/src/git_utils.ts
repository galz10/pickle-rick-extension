...
import { printMinimalPanel, run_cmd } from './pickle_utils.js';
...
export function create_worktree(repo_path: string, base_branch: string, task_id: string): string {
    ...
    if (fs.existsSync(worktree_path)) {
        ...
        try { run_git(["worktree", "remove", "--force", worktree_path], undefined, false); } catch {}
        if (fs.existsSync(worktree_path)) {
    ...
    try { run_git(["branch", "-D", new_branch], repo_path, false); } catch {}
...
export function remove_worktree(worktree_path: string): void {
