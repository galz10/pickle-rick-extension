...
export function printMinimalPanel(title: string, fields: Record<string, string | number | boolean | null | undefined>, colorName: StyleColor = "GREEN", icon: string = "🥒") {
...
export function run_cmd(cmd: string | string[], options: { cwd?: string, check?: boolean, capture?: boolean } = {}): string {
    ...
    try {
        ...
        if (check) throw new Error(`Command failed: ${command}\nError: ${err.stderr?.toString() || err.message}`);
        return err.stdout?.toString().trim() || '';
    }
}

export async function spawn_cmd(cmd: string[], options: { cwd?: string, onData?: (data: string) => void } = {}): Promise<number> {
